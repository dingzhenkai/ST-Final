...
1. 弱一般等价类 MAX（N,M）
2. 强一般等价类 N*M
3. 弱健壮等价类/改进的弱健壮等价类 MAX(N,M)+2 / MAX(N,M)+2x
4. 强健壮等价类 (N+2)*(M+2)
...
1. 黑盒测试：根据程序外部特征进行测试
2. 白盒测试：根据程序内部结构逻辑进行测试
3. 在进行单元测试时大多采用白盒测试，在进行集成测试或系统测试时大多采用黑盒测试。
...
1. 由一个节点组成，入度为0
2. 由一个节点组成，出度为0
3. 由一个节点组成，入度大于等于2或出度大于等于2
4. 由一个节点组成，入度和出度均为1
5. 长度大于等于1的最大链（单入单出的最大序列）
...
集成测试是结构化测试，需要了解程序的结构，需要软件开发人员的参与，有路径覆盖的意义。

系统测试是一种黑盒测试，不需要了解软件结构，用户参与。
...
原子系统功能（ASF）是指在系统级上可观测的行为，可以用端口输入和端口输出事件来描述。
...
每当软件发生变化时，我们就必须重新测试现有的功能，以便确定软件修改是否达到了预期的目的，检查修改是否损害了原有的功能。同时还需要补充新的测试用例来测试新的或被修改了的功能，为了验证修改的正确性及其影响，就需要进行回归测试。
